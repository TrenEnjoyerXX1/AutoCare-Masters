...
<div class="depart" style="width: 100%;  font-family: 'Times New Roman', Times, serif;">
    <h2><span style="display:flex;  text-align: center; color:white;" ><span style="align-items:center; width:100%;">Car Wrap Department</span></span></h2>
    ...
    while ($row = $result->fetch_assoc())
    {
        ...
        <tr style="color: white; border: 1px solid black;">
            ...
            <th> <?php  if($row['Status']!="Completed"){ ?>
                <form method="post" action="ShopUpdateCarWrap.php"> <input class="btn btn-danger" type="submit" value="Cancel" name="<?php echo $row['r_id']?>" > </form>
                <br>
                <form method="post" action="ShopUpdateCarWrap.php" > <input class="btn btn-success" type="submit" value="Approve" name="<?php echo $row['r_id']?>" > </form>
                <?php } ?>   </th>
        </tr>
